...
Moderation-warning pushes can hide the denied inbound message from history and future context when the request metadata includes `source: "moderation_warning"` plus an explicit source message identifier, preferably `inbound_message_id` or `nexus_message_id`; `message_id` is accepted for compatibility with the Laju inbound metadata contract. Nexus does not hide the latest inbound message as a fallback, because outbound moderation notices may arrive after newer customer messages.
