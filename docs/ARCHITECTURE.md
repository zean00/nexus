...
The admin API can enqueue outbound-only reminders, broadcasts, and notifications from an external scheduler or agent runtime. Push requests resolve targets by ACP session ID, explicit Nexus session ID, notification surface, channel account, or linked identity, then reuse the same renderer, delivery table, and worker path as ACP run output. `acp_session_id` is the preferred field; legacy `session_id` is interpreted as the ACP session ID for Duraclaw compatibility. Nexus fans out to every Nexus session mapped to that ACP session unless `channel_type` filters delivery to one channel. Responses include per-target delivery IDs and channel availability so callers can distinguish “webchat delivered, WhatsApp unavailable” from generic push failure. Operators can inspect the mapping through `GET /admin/sessions/by-acp?acp_session_id=...`, optionally filtered by `channel_type`.
