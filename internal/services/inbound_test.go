...
func TestInboundServiceNormalizesLocalWhatsAppPairingNumber(t *testing.T) {
	repo := &fakeRepo{
		receipts: map[string]bool{},
		sessions: map[string]domain.Session{},
	}
	...
	raw, _ := json.Marshal(map[string]string{"ok": "true"})
	result, err := svc.Handle(context.Background(), domain.CanonicalInboundEvent{
		EventID:         "evt_link_wa_local",
		TenantID:        "tenant_default",
		Channel:         "whatsapp",
		ProviderEventID: "provider_link_wa_local",
		ReceivedAt:      time.Now(),
		Sender:          domain.Sender{ChannelUserID: "0812-3456-7890"},
		Conversation:    domain.Conversation{ChannelSurfaceKey: "wa_surface_1"},
		Message:         domain.Message{MessageID: "msg_link_wa_local", Text: "link user_1.12345678"},
		Metadata:        domain.Metadata{RawPayload: raw},
	})
	if err != nil {
		t.Fatal(err)
